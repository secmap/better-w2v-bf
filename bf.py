...
class bloomfilter():
    ...
    def get_word_in_table(self, table_id, opcode_hash):
        return self.tables[table_id][opcode_hash]
    
    def get_possible_words_by_indices(self, indices):
        possible_words = set()
        for idx, val in enumerate(indices):
            if idx == 0:
                possible_words = self.get_word_in_table(idx, val)
            else:
                possible_words = possible_words & self.get_word_in_table(idx, val)
